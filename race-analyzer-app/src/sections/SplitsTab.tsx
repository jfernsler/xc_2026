...
type ViewMode = "splits" | "segment-stats" | "distribution" | "vs-average";
...
export function SplitsTab({ raceName, filtered, splits, hl }: SplitsTabProps) {
  ...
  const segmentMeans = useMemo(
    () => segmentStatsPerSegment.map((s) => s?.mean ?? null),
    [segmentStatsPerSegment]
  );
      ...
      {viewMode === "vs-average" && (
        <div className="space-y-6">
          {Object.entries(grouped).map(([cat, riders]) => (
            <div key={cat}>
              <h2 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-1">
                {cat} — vs segment average ({timingMode})
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Green = faster than average, red = slower.</p>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-slate-400 dark:text-slate-500 border-b border-slate-200 dark:border-slate-600">
                      ...
                      return (
                        ...
                        >
                          ...
                          {vals.map((v, i) => {
                            ...
                            return (
                              <td key={i} className={cellClass} title={delta != null ? `${delta >= 0 ? "+" : ""}${formatTime(delta)} vs avg` : ""}>
